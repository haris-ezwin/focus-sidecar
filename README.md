...
Codex usage requires the Codex CLI signed in with a ChatGPT account. Sidecar reads quota metadata through `codex app-server` every minute while running; it does not start model turns. It saves readings and up to seven observed days in `~/Library/Application Support/Focus Sidecar/codex-usage.json`. Days start at midnight Asia/Kuala_Lumpur. Each day starts from the first timestamped reading after midnight Malaysia time, counting only subsequent increases. The details show the start time and weekly baseline. Existing totals without a baseline restart on the next successful reading. Changes to the reset timestamp establish a new baseline without adding the weekly total. Downward corrections within a window do not count again on rebound. Days are marked partial because usage before the baseline or across resets cannot be reconstructed. Counts are approximate to the quota readings and polling interval. The bar turns amber at 80% of the daily target and red at 100%; connection failures show a muted bar and details instead of claiming a fresh reading. This target is a personal budget, not a separate Codex limit.
